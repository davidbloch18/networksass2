...
BUFFER_SIZE = (
    65536  # The buffer size is the maximum amount of data that can be received at once
)
...
class CalculatorHeader:
    ...
    def unpack(cls, data: bytes) -> "CalculatorHeader":
        ...
        return cls(
            unix_time_stamp=unix_time_stamp,
            ...
            data=data[cls.HEADER_MIN_LENGTH :],
        )

    @classmethod
    def from_request(
        cls, data: bytes, show_steps: bool, cache_result: bool, cache_control: int
    ) -> "CalculatorHeader":
        return cls(
    ...
    @classmethod
    def from_expression(
        cls, expr: Expression, show_steps: bool, cache_result: bool, cache_control: int
    ) -> "CalculatorHeader":
    ...
    def from_result(
        ...
        return cls.from_response(
            data=pickle.dumps((result, steps)),
            status_code=CalculatorHeader.STATUS_OK,
    ...
    @classmethod
    def from_error(
        cls, error: Exception, status_code: int, cache_result: bool, cache_control: int
    ) -> "CalculatorHeader":
        return cls.from_response(
...
def data_to_result(header: CalculatorHeader) -> typing.Tuple[numbers.Real, list[str]]:
    try:
        ...
        if (
            ...
            or len(result) != 2
            or not isinstance(result[0], numbers.Real)
