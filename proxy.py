...
def process_request(
    request: api.CalculatorHeader, server_address: tuple[str, int]
) -> tuple[api.CalculatorHeader, int, int, bool, bool, bool]:
    """
    ...
    if ((data, request.show_steps) in cache) and (request.cache_control != 0):
        ...
        server_time_remaining = int(res_cc - age)
        client_time_remaining = int(req_cc - age)
        ...
        while True:
            try:
                ...
                thread = threading.Thread(
                    ...
                    args=(client_socket, client_address, server_adress),
                )
                thread.start()
        ...
        while True:
            ...
            try:
                ...
                (
                    ...
                    was_stale,
                    cached,
                ) = process_request(request, server_address)
                ...
                print(
                    f"{client_prefix} Sending response of length {len(packed_response)} bytes"
                )
